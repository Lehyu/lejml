...
public class LogisticsRegressionTest {
    ...
    public static void main(String[] args) throws IOException {
        ...
        SGDOptimizer optimizer = new SGDOptimizer.Builder().batch(20).eta(0.1).maxIter(1000).loss(LossUtils.LossEnum.LOG_LOSS.getName()).lambda(1e-3).earlyStopping(10).build();
        LinearModel.Builder builder = new LinearModel.Builder().fitIntercept(true).normalize(true).optimizer(optimizer);
        LogisticsRegression lr = new LogisticsRegression(builder);
        lr.fit(X, y);
